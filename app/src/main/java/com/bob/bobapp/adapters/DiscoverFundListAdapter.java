...
public abstract class DiscoverFundListAdapter extends RecyclerView.Adapter<DiscoverFundListAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if(lstRecommandationDebtArrayList.get(position).getMorningStarRating()!=null) {
            if (lstRecommandationDebtArrayList.get(position).getMorningStarRating().equalsIgnoreCase("0")) {
                holder.linearRating.setVisibility(View.GONE);
            }
            else
                {
                holder.linearRating.setVisibility(View.VISIBLE);
                holder.txtRating.setText(lstRecommandationDebtArrayList.get(position).getMorningStarRating());
            }
        }
        holder.txtClassification.setText(lstRecommandationDebtArrayList.get(position).getClassification());

        if (lstRecommandationDebtArrayList.get(position).getReturnIn3Month().equalsIgnoreCase("0.0")) {
            holder.txtThreeMonth.setText("-");
        } else {
            double threeMonth = Double.parseDouble(lstRecommandationDebtArrayList.get(position).getReturnIn3Month());
        ...
        if (lstRecommandationDebtArrayList.get(position).getReturnIn1Year().equalsIgnoreCase("0.0")) {
            holder.txtOneYear.setText("-");
        } else {
            double oneYear = Double.parseDouble(lstRecommandationDebtArrayList.get(position).getReturnIn1Year());
        ...
        if (lstRecommandationDebtArrayList.get(position).getReturnIn6Month().equalsIgnoreCase("0.0")) {
            holder.txtSixMonth.setText("-");
        } else {
            double sixMonth = Double.parseDouble(lstRecommandationDebtArrayList.get(position).getReturnIn6Month());
        ...
        if (lstRecommandationDebtArrayList.get(position).getReturnIn3Year().equalsIgnoreCase("0.0")) {
            holder.txtThreeYear.setText("-");
        }
        else {
            double threeYear = Double.parseDouble(lstRecommandationDebtArrayList.get(position).getReturnIn3Year());

            threeYear = util.truncateDecimal(threeYear).doubleValue();

            holder.txtThreeYear.setText(""+threeYear);
        }
        if (lstRecommandationDebtArrayList.get(position).getReturnIn5Year() != null) {
            if (lstRecommandationDebtArrayList.get(position).getReturnIn5Year().equalsIgnoreCase("0.0")) {
                holder.txtFiveYear.setText("-");
            } else {
                double fiveYear = Double.parseDouble(lstRecommandationDebtArrayList.get(position).getReturnIn5Year());

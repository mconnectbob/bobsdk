...
public abstract class EquityAdapter extends RecyclerView.Adapter<EquityAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        if(lstRecommandationEquityArrayList.get(position).getMorningStarRating()!=null) {
            if (lstRecommandationEquityArrayList.get(position).getMorningStarRating().equalsIgnoreCase("0")) {
                holder.linearRating.setVisibility(View.GONE);
            } else {
                holder.linearRating.setVisibility(View.VISIBLE);
                holder.txtRating.setText(lstRecommandationEquityArrayList.get(position).getMorningStarRating());
            }
        }
        holder.txtFundName.setText(lstRecommandationEquityArrayList.get(position).getFundName());
        holder.txtClassification.setText(lstRecommandationEquityArrayList.get(position).getClassification());

        if (lstRecommandationEquityArrayList.get(position).getReturnIn3Month().equalsIgnoreCase("0.0")) {
            holder.txtThreeMonth.setText("-");
        } else {
            double threeMonth = Double.parseDouble(lstRecommandationEquityArrayList.get(position).getReturnIn3Month());
        ...
        if (lstRecommandationEquityArrayList.get(position).getReturnIn1Year().equalsIgnoreCase("0.0")) {
            holder.txtOneYear.setText("-");
        } else {
            double oneYear = Double.parseDouble(lstRecommandationEquityArrayList.get(position).getReturnIn1Year());
        ...
        if (lstRecommandationEquityArrayList.get(position).getReturnIn6Month().equalsIgnoreCase("0.0")) {
            holder.txtSixMonth.setText("-");
        } else {
            double sixMonth = Double.parseDouble(lstRecommandationEquityArrayList.get(position).getReturnIn6Month());
        ...
        if (lstRecommandationEquityArrayList.get(position).getReturnIn3Year().equalsIgnoreCase("0.0")) {
            holder.txtThreeYear.setText("-");
        }
        else {
            double threeYear = Double.parseDouble(lstRecommandationEquityArrayList.get(position).getReturnIn3Year());

            threeYear = util.truncateDecimal(threeYear).doubleValue();

            holder.txtThreeYear.setText(""+threeYear);
        }
        if (lstRecommandationEquityArrayList.get(position).getReturnIn5Year() != null) {
            if (lstRecommandationEquityArrayList.get(position).getReturnIn5Year().equalsIgnoreCase("0.0")) {
                holder.txtFiveYear.setText("-");
            } else {
                double fiveYear = Double.parseDouble(lstRecommandationEquityArrayList.get(position).getReturnIn5Year());
    ...
    class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        private TextView txtFundName, txtSixMonth, txtThreeMonth, txtOneYear,
                txtBuy,txtSip,txtClassification,txtThreeYear, txtFiveYear,txtRating;
